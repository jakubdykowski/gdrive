...
public class DownloadTransferHandler extends TransferHandler
{
  ...
  public boolean canImport(TransferHandler.TransferSupport info)
  {
    ...
    if (!info.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) { System.out.println(" |false");
      return false;
    }
    return true;
  }
  ...
  {
    System.out.print("");
    if (!info.isDrop()) { System.out.println("");
      return false;
    }
    ...
    java.io.File[] file = new java.io.File[li.size()];
    for (int i = 0; i < li.size(); i++) {
      file[i] = ((java.io.File)li.get(i));
    }
    System.out.println(file[0].getAbsolutePath());
    if (this.component.isfile) {
      File[] pliki = { Main.dragged_file };
      new Download(pliki, null).start();
    }

    Main.window.downloaded.updateUI();
    Main.dragged_file = null;
    Main.dragged_temp = null;
